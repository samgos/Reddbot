...
class Command
  ...
  ACTIONS = %w(leaderboard balance chart rain deposit tip withdraw about price $
  def initialize(slack_params)
    @coin_config_module = Kernel.const_get ENV['COIN'].capitalize
    text = slack_params['text']
    @params = text.split(/\s+/)
    raise "REDDCOIN IS THE BEST" unless @params.shift == slack_params['trigger_$
